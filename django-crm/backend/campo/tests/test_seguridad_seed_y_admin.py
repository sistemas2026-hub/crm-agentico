...
def test_en_desarrollo_siembra_sobre_la_organizacion_indicada(org_a):
    ...
    orden = OrdenTrabajo.objects.get(org=org_a)
    assert orden.estado_operativo == OrdenTrabajo.ASIGNADA
...
def test_la_orden_sembrada_se_declara_demo_y_no_finge_venir_de_wisphub(org_a):
    ...
    orden = OrdenTrabajo.objects.get(org=org_a)
    assert orden.origen_sistema == "demo"
    assert orden.origen_sistema != "wisphub"
    assert "DEMOSTRACION" in orden.diagnostico_previo["resumen"].upper()
...
def test_dos_corridas_no_chocan_con_el_consecutivo(org_a):
    ...
    numeros = sorted(OrdenTrabajo.objects.filter(org=org_a)
