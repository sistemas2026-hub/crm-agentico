...
export const actions = {
  ...
  async create(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { create: { error: 'Solo un administrador puede cambiar políticas de escalamiento.' } });
      }
      return fail(400, { create: { error: readableError(err, 'No se pudo agregar la política.') } });
    }
    return { created: true };
  },
  ...
  async update(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { update: { error: 'Solo un administrador puede cambiar políticas de escalamiento.' } });
      }
      return fail(400, { update: { error: readableError(err, 'No se pudo guardar la política.') } });
    }
    return { updated: true };
  },
  ...
  async deactivate(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, {
          deactivate: { error: 'Solo un administrador puede cambiar políticas de escalamiento.' }
        });
      }
      return fail(400, {
        deactivate: { error: readableError(err, 'No se pudo apagar la política.') }
      });
    }
  ...
  async activate(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { activate: { error: 'Solo un administrador puede cambiar políticas de escalamiento.' } });
      }
      return fail(400, {
        activate: { error: readableError(err, 'No se pudo encender la política.') }
      });
    }
  ...
  async remove(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { remove: { error: 'Solo un administrador puede cambiar políticas de escalamiento.' } });
      }
      return fail(400, { remove: { error: readableError(err, 'No se pudo eliminar la política.') } });
    }
    return { removed: true };
  }
};
