...
export const actions = {
  ...
  async create(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { create: { error: 'Solo un administrador puede cambiar reglas de aprobación.' } });
      }
  ...
  async update(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { update: { error: 'Solo un administrador puede cambiar reglas de aprobación.' } });
      }
  ...
  async deactivate(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { deactivate: { error: 'Solo un administrador puede cambiar reglas de aprobación.' } });
      }
      return fail(400, {
  ...
  async activate(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { activate: { error: 'Solo un administrador puede cambiar reglas de aprobación.' } });
      }
      return fail(400, {
  ...
  async remove(event) {
    ...
    try {
      ...
      if (err?.status === 403) {
        return fail(403, { remove: { error: 'Solo un administrador puede cambiar reglas de aprobación.' } });
      }
