...
Lo que hace falta cuando se retome: confirmar si `deepseek-v4-flash` acepta imágenes (si no, hace falta un segundo modelo solo para esto, con su costo y su latencia), y una regla que no se puede saltear — lo que se lea se **compara** contra la cédula del titular en WispHub, nunca se acepta como "parece válida". Una foto de WhatsApp movida o con brillo se lee mal, y equivocarse acá es o bloquear a un cliente legítimo o dejar pasar a un impostor. Si no se puede leer o no coincide: va a una persona, nunca a una suposición.
...
**Modelo documental: lo que se decidió NO hacer todavía (22/08/2026).** Al agregar la aprobación de documentos y el guardado del archivo original (`supabase/202608231328_aprobacion_documentos` y `supabase/202608231459_original_documentos`) se evaluó un modelo más completo y se dejó afuera a propósito. Queda anotado para cuando aparezca la presión que lo justifique, y con el motivo por el que hoy no se hizo:

- **Tabla `document_versions` separada, con los chunks apuntando a la versión.** Es la forma canónica, pero el eje de versión **ya existe**: `asistente.documents` se indexa por `codigo + version`, así que subir la v02 crea una fila nueva y la v01 queda intacta. Lo que faltaba —que no se pudieran pisar los bytes de una versión ya aprobada— se resolvió con una regla en `ingerir()`. Migrar `document_chunks.document_id` → `document_version_id` obligaría a tocar `match_chunks`, que corre en **cada mensaje de cada cliente**. Hacerlo el día que convivan varias versiones del mismo código, no antes.
- **Separar `approval_status` de `ingestion_status`.** Hoy `estado` mezcla las dos cosas (`pendiente`/`vigente`/`obsoleto`). Conceptualmente son distintas —un documento puede estar aprobado y haber fallado la vectorización— pero hoy la ingesta es síncrona y si falla lanza excepción: ese estado no existe. Vale la pena el día que la ingesta pase a una cola.
- **Dos listas de acceso distintas.** Una cosa es *"¿puede el asistente usar este contenido?"* (los `roles_permitidos` de hoy) y otra *"¿puede esta persona descargar el `.docx` completo, con firmas y anexos?"*. No aplica todavía porque no hay ninguna forma de descargar el original — el día que se agregue ese botón, no reusar la primera lista para la segunda.
- **Mover los bytes a un almacenamiento de objetos.** Hoy van en `bytea` (~20 documentos de 30 KB a 2 MB, y ya existía el precedente de `asistente.media`). Se reconsidera si aparecen miles de documentos, archivos mucho mayores, o el backup de Postgres empieza a molestar. Cuando pase, en la tabla queda la referencia y **la semántica de "esta versión fue aprobada" no cambia**: es lo que hay que preservar en esa migración.
...
2. Verificado con el backend local apuntando a la base real (ver "Desarrollo local contra la base real"), sin pasar por Dokploy: `manage_rls --verify-user` confirma `Is superuser: False` bajo `crm_user`. **Corrección (12/09/2026): esta frase decía «a diferencia de `--status`, este SÍ pregunta lo correcto», y era falsa.** `--verify-user` consultaba `pg_user.usesuper` igual que `--status`, así que tenía exactamente el mismo punto ciego: un rol con `rolbypassrls=true` y `rolsuper=false` —la forma del `postgres` de Supabase— pasaba como seguro. Los dos se corrigieron: ahora preguntan `rolsuper` **y** `rolbypassrls` en `pg_roles`, informan `current_user` y `session_user`, tratan «no se pudo comprobar» como fallo y terminan con código distinto de cero. Medido en base efímera sobre un rol `NOSUPERUSER BYPASSRLS`: `pg_user.usesuper` devuelve `f` (es decir, «seguro») y `rolbypassrls` devuelve `t`. `--test` no corre (pide 2 organizaciones y hoy hay 1), así que se probó a mano: con `app.current_org` puesto al id real, `crm_user` ve exactamente las mismas filas que ve `postgres` sin ninguna restricción (11 `cases`, verificado); sin contexto, 0 filas — el fail-safe funciona.
...
**El ciclo hace tres cosas, en este orden, y cada una aislada.** Por tenant: (A) cerrar conversaciones vencidas, (B) descubrir e importar tickets, (C) reconciliar los `external_*` de los casos que ya existen. A la corre el reloj; B y C viven las dos dentro de `importacion_io.barrido()`.

C se cableó el 10/09/2026, **después** de auditar el primer ciclo automático real: creó los 3 casos esperados y reconcilió **cero**. La reconciliación estaba construida, probada y sin nadie que la llamara — `aplicar_reconciliacion` solo se alcanzaba desde el CLI. Es el mismo bug que el reloj muerto, en chico. Guarda: `tests/test_reloj.py` corre el `barrido` de verdad entrando por el reloj y exige que la reconciliación se ejecute en el ciclo.

Aislamientos, los cuatro medidos: una importación rota no impide reconciliar (el paso B entero va en su `try`, no solo el listado); una reconciliación rota no revierte lo importado ni corta al tenant siguiente; ninguna de las dos mata los vencimientos; ninguna mata el proceso. Y los contadores van **separados** — `importacion: {inspeccionados, candidatos, creados, ya_estaban, fallidos}` y `reconciliacion: {alcanzados, con_diferencia, actualizados, sin_cambios, fallidos, errores_lectura}` — sin ningún número plano arriba que los sume, que es el que se lee mal justo cuando algo anda mal.

**`cada_horas` manda sobre el subsistema entero.** En 0 no se importa **ni se reconcilia**. Es la única semántica explicable en una frase; la alternativa dejaba un estado incómodo ("no importa nada pero igual habla con el proveedor cada hora") y un segundo interruptor que recordar. Si algún día hace falta refrescar sin importar, eso es una clave nueva y explícita en la config, no una excepción escondida en el código.
...
**Deuda: `editor._editar` reescribe defaults ausentes (10/09/2026).** Una mutación de UNA clave (`importacion_tickets.cada_horas: 0 → 1`) produjo un documento con **68** hojas distintas. Las otras 67 fueron `herramientas[0..66].consulta_guias_tv: <no estaba> → false`: el campo entró al schema ese mismo día (`0fa503a`) con default `False`, y `_editar` valida y **reserializa el documento entero** por Pydantic, así que todo campo nuevo se materializa con su default en cada objeto aunque nadie lo pida.

Fue semánticamente neutro —ninguna herramienta lo tenía, ninguna quedó en `true`, y se verificó una por una que las 67 no cambiaron en ningún otro campo— y por eso la v138 **no se revirtió**. Importa igual por dos razones: el diff de una edición mínima deja de ser legible (auditar "qué cambió" pasa a ser buscar una aguja entre 67 líneas de ruido), y un default nuevo se aplica retroactivamente sin que nadie lo decida.

Qué evaluar cuando se retome: `model_dump(exclude_unset=True)` en el camino de escritura, o —probablemente lo correcto— aplicar la mutación como **patch sobre el documento original** y usar el modelo solo para *validar* el resultado, no para producir lo que se guarda. Con una prueba que edite una sola clave y exija exactamente una hoja distinta en el diff.
