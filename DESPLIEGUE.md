...
**Activado en producción (18/08/2026).** `DBUSER`/`DBPASSWORD` son una sola variable compartida por todo el stack (Docker Compose, no un panel por servicio como se pensaba al principio), así que el corte fue un solo cambio, no tres. Primer intento falló el build (`error while interpolating services.backend.environment.DBPASSWORD: required variable DBPASSWORD is missing a value`) por escribir `DBUSER:...` con dos puntos en vez de `DBUSER=...` — un `.env` no es YAML. Corregido, redesplegado, verificado desde afuera sin acceso a los logs de Dokploy: `https://agent.rapilinksas.co/login` y `https://agent-api.rapilinksas.co/admin/` cargan normal, sin 500/502 ni traceback — si `migrate` hubiera fallado bajo el rol nuevo (que no es dueño de las tablas), gunicorn no habría llegado a levantar y esto daría 502. Confirmado también en local con el mismo `.env`: `current_user` es `crm_user`, `rolbypassrls=false`.

Sigue en pie el pendiente de fondo: el día que una migración de BottleCRM necesite activar RLS en una tabla nueva, va a fallar bajo este rol porque no es dueño de las tablas (`postgres` sí). No es una tarea de hoy — se resuelve separando la credencial de migración de la de tráfico normal cuando haga falta.
