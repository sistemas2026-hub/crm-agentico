...
**Autenticar `/chat` y `/agentes` en el motor — MITAD HECHA (agosto 2026).** Hoy `/chat`, `/agentes` y el resto de rutas internas no pedían nada: lo único que las separaba de internet era el `PathPrefix` de una regla de Traefik, una sola capa, cuando el resto del proyecto usa dos por principio (PRD §7.4). El lado del **motor** ya está: `nucleo/canales/api.py::_exigir_token_de_servicio()` rechaza con 401 cualquier ruta que no sea `/salud` o el webhook de WhatsApp (esas dos se autentican con otro mecanismo — healthcheck sin credenciales y firma de Meta, respectivamente) si no llega el header `X-Servicio-Token` con el valor de `MOTOR_SERVICE_TOKEN`. Guarda: `tests/test_token_servicio.py`.
...
⚠️ **El lado del frontend queda SIN HACER, y es la mitad que importa para activar esto.** `django-crm/frontend` le habla al motor desde **18 archivos, ~40 llamadas `fetch()`**, sin un cliente HTTP compartido — cada ruta arma la suya. Wireear el header ahí exige poder levantar el CRM y probar cada pantalla afectada (ajustes, agentes, conversaciones, simulador de WhatsApp, manual, corpus), que no se pudo hacer en la sesión donde se agregó el lado del motor. **No cargar `MOTOR_SERVICE_TOKEN` en las variables de Dokploy hasta que el frontend esté wireado** — si se activa el gate en el motor sin que el frontend mande el header, las 18 pantallas se rompen todas a la vez con 401.
