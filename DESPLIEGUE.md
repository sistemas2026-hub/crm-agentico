...
**Para activarlo en producción**, una vez decidido lo anterior — mismo patrón que `MOTOR_SERVICE_TOKEN`: en Dokploy, servicios `backend`, `celery-worker`, `celery-beat`, cambiar `DBUSER` a `crm_user` (con el sufijo del pooler: `crm_user.05b5a4b4-3b9a-4901-86f2-f3ed8f8ac0a1`) y `DBPASSWORD` al valor generado (entregado aparte, nunca commiteado). Redesplegar los tres juntos — si uno queda con `postgres` y otro con `crm_user`, empiezan a ver datos distintos sin ningún error visible.
