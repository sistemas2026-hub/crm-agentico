...
**Versión:** 3.1 (18/09/2026): v3.0 + B3.5 (D18): la cola se proyecta en el backend por bandas explícitas, con `esperando_desde` y el motivo en palabras (§4.10, C18). v3.0: v2.9 + D27 corregido (la hora del evento se toma después del lock) y D28 registrado para B4 (la asignación del ticket del CRM puede divergir; Dexter manda). v2.9: B3.4 (D4 cerrado: toma atómica, soltar solo el dueño, reasignación ADMIN auditada; C16, C17) y D27 (el orden de los eventos es la versión). v2.8: la sincronización de una escalada comprometida se autoriza por la escalada, no por la versión exacta (C15), e inventario vivo de efectos (§4.8). v2.7: D25 (ningún efecto que escribe empieza después de un cambio de control; inventario de efectos) y `descartado` terminal. v2.6: D24 (revalidación del control al volver del modelo y antes del POST; punto de no retorno) y replay idempotente solo para el mismo actor y operación. v2.5: B3.3b (compuerta durable del modelo, T8 Intervenir) y G1/G9 en verde. **Estado: CERRADO como arquitectura** después de dos rondas de auditoría. Autoriza preparar la implementación por fases (§15) en `feature/bandeja-relevo`, **sin push a producción**. Cada fase vuelve a auditoría antes de integrarse.
...
| C9 | **G1 y G9 en verde en producción.** G9 deja de ser un gate pendiente: `wamid` persistido (G9-A) y acuse de entregado correlacionado (G9-B). **Que G9 esté verde no activa T6**: T6 entra con el control durable, en su fase. | §16.1, T6 |
| C10 | **D24. Un turno de la IA revalida su autorización fuera de transacción.** Al autorizar se guarda `conversation_id` y `relevo_version`. (1) Al volver del modelo, antes de guardar la respuesta o tocar la memoria: conversación abierta, la misma, `control_efectivo = 'ia'` y la misma versión; si no, la salida se descarta (ni fila, ni memoria, ni envío), el mensaje del cliente sí se guarda y se cuenta `respuesta_ia_descartada_por_cambio_de_control` sin contenido. (2) Justo antes del POST a Meta: la misma conversación y la versión esperada (la del propio turno si escaló); si no, no se envía y la fila queda `estado_entrega = 'descartado'`, fuera del historial y del resumen. Si la base no responde, falla cerrado y se cuenta `control_no_determinado`. Un legado retomado por cierre del CRM compara solo la versión (sus banderas siguen en la base hasta G8). | X3, I20, §16.1b P-A |
| C11 | **Punto de no retorno.** Una intervención impide todo envío de la IA cuyo POST al proveedor todavía no empezó cuando la intervención quedó durable. Un POST ya en vuelo puede completar. Nunca se sostiene una transacción esperando al proveedor; la garantía absoluta ("ningún mensaje después del clic") pediría una reserva durable de salida (outbox), fuera de esta fase. | T8, P-A |
| C13 | **D25. Ningún efecto que escribe, originado por un turno, empieza después de un cambio de control que lo deje sin autorización.** Mientras corre el turno, `atender_turno` instala un autorizador; cada efecto le pregunta **justo antes de empezar** y se relee la base, fuera de transacción. Las reglas de las tres clases viven en un solo lugar, `nucleo/relevo/autorizacion.py` (§4.8). Si no se cumple: no empieza, un efecto autónomo no se reintenta en el turno, la traza lo marca `CAMBIO_DE_CONTROL` (invariante: no fuerza escalada) y se cuenta `accion_ia_cancelada_por_cambio_de_control` con `registrar()` de D20, sin contenido. Las lecturas no preguntan. Punto de no retorno de C11: una llamada ya empezada completa. Fuera de un turno no hay autorizador: la aprobación humana de una acción (B5) tiene su propia revalidación. | T8, X3, C11, §4.8 |
| C15 | **La obligación de sincronizar una escalada nace con la escalada.** El ticket y el caso del CRM de una escalada que el turno ya dejó durable se autorizan por **esa escalada todavía vigente** (su evento existe, la conversación sigue abierta y en manos de personas, y después no hubo `devuelta_a_ia` ni `cerrada`), no por la `relevo_version` exacta. Tomar, soltar o reasignar cambian quién atiende y **no** la cancelan; devolver o cerrar antes de que el efecto empiece, sí. El aviso de esa escalada al cliente (D24, punto 2) sigue la misma regla. B4 la vuelve sincronización durable y reconciliable, **por efecto**: el punto de no retorno permite un resultado parcial (ticket hecho, caso nunca empezado) y una escalada nunca se da por «sincronizada» como un todo. Pendiente de UX para B3.5: si una persona escribe antes de que salga el aviso de la escalada, decidir si el aviso sobra. | T1, C13, §4.8 |
| C16 | **B3.4 (D4). La asignación tiene una sola puerta.** Toda escritura de `asignada_a_*`/`tomada_por` pasa por `nucleo/relevo/transiciones.py` con la fila bloqueada (`FOR UPDATE`), precondición, escritura y evento en una transacción corta, sin efectos externos (inventario: `tests/test_asignacion_escritores.py`). **Tomar:** solo con `control_efectivo = 'humano'`, abierta y libre; si ya tiene dueño, 409 `ya_asignada` con quién la tiene, nunca se pisa; bajo control IA, 409 `control_ia` (se usa Intervenir). **Soltar:** solo el asignado; otro, 409 `no_es_suya`. **Reasignar (T4):** operación propia, solo `ADMIN` (403 si no), motivo obligatorio (400), destino validado contra las personas activas de la organización desde la sesión; evento `reasignada` con actor, anterior (id y nombre), nuevo (id y nombre), motivo y versión. Idempotencia C12 en las tres. La pantalla no cambia la propiedad por un clic: espera el 200 y, ante un 409, relee y dice quién quedó. | T2, T3, T4, X5 |
| C17 | **Legado (`relevo_version = 0`) hasta G8:** tomar y soltar aplican la misma exclusión sobre `tomada_por` (por nombre), sin evento ni versión, y solo si `control_efectivo` es humano. Reasignar un legado es 409 `legado_sin_relevo`: una reasignación exige auditoría y el legado no tiene eventos; se adopta primero en G8. Ningún clic adopta una conversación. | C5, I21, G8 |
| C18 | **B3.5 (D18). La cola se calcula en el backend, por bandas, no con un puntaje en la pantalla.** `nucleo/relevo/proyeccion.py` deriva de la verdad durable: `necesita_accion_de`, `banda` (§4.10), `esperando_desde`, `motivo_cola` (en palabras, para que el operador sepa por qué una fila está arriba) y `es_legado`; `/conversaciones` las devuelve ya calculadas y ordenadas. La pantalla ordena por (banda, `esperando_desde`) y muestra el motivo; no reimplementa la regla. Es **solo lectura**: no adopta legado (G8), no escribe, no usa `atendida_manual` ni el dueño del ticket del CRM (D28), y no ordena eventos por `creado_en` (D27). Vista operativa por defecto: solo `canales.REALES`. | §4.2, §4.5, §4.10, D18 |
| C14 | **`estado_entrega = 'descartado'` es terminal.** Dexter decidió no enviar: no es `fallido`. Nunca se ofrece reintentar (la pantalla reintenta solo `fallido`; el reintento del motor es de mensajes humanos), nunca avanza por acuses (solo se marca sin `wamid`, y los acuses llegan por `wamid`), no entra al historial ni al resumen, no cuenta como entrega al cliente; el consumo del modelo sí queda medido. | C10 |
| C12 | **Replay idempotente = misma operación y mismo actor.** Una clave ya usada devuelve reintento solo si coinciden el tipo de evento y el actor; si no, `clave_ajena` (en `/intervenir`, 409 `clave_de_otra_operacion`) y nada cambia. | §3.3, T8 |
...
### 4.10 Las bandas de la cola [AUDITORÍA D18, C18]
`Recomendado` ordena por **banda** y, dentro de una banda, por `esperando_desde` más antiguo primero (sin eso, una banda con tráfico deja a alguien esperando para siempre). No hay puntaje: una fila está donde está por una razón que se puede leer.

| Banda | Situación | `necesita_accion_de` | `esperando_desde` |
|---|---|---|---|
...
### 4.9 El orden de los eventos [AUDITORÍA D27]
`datos.version` es el orden **causal** de los eventos de una conversación: lo fija la transición bajo el lock de la fila. `creado_en` es la hora que se **muestra**, tomada con `clock_timestamp()` al insertar (ya con la fila bloqueada), así que en los eventos nuevos las dos coinciden. Ninguna lectura —la línea de tiempo y la cola de B3.5 incluidas— ordena por `creado_en`.

### 4.8 Inventario vivo de efectos del turno [AUDITORÍA D25, C13, C15]
Regla en `nucleo/relevo/autorizacion.py`; un efecto nuevo entra a una de estas clases o la suite `tests/test_relevo_efectos.py` falla.

| Clase | Qué exige antes de empezar | Efectos (hoy) |
|---|---|---|
| **Solo lectura** | Nada | Consultas de cliente, facturas, tickets y planes; señal, estado de ONT, ping, estabilidad, incidentes; verificación y confirmación de identidad; guías, documentación, habilidades; derivación de área (solo estado de sesión); agregados e informes |
| **`AUTONOMO_IA`** | `control_efectivo = 'ia'` + la misma `relevo_version` | Toda herramienta con `solo_lectura: false` que pide el modelo (reiniciar ONT, activar CATV, cambiar tipo de ONU, registrar pago, solicitud de servicio, pedido WiFi, comprobante de pago, tickets y casos autónomos…); propuestas con aprobación humana; `proponer_herramienta`; visita agendada sola; cierre por confirmación del cliente (y el supervisor, solo si cerró); ticket y caso de una escalada que **no** reservó este turno |
| **`SYNC_ESCALADA`** | La escalada originadora vigente (C15) | Ticket operativo y caso del CRM de la escalada que este turno dejó durable; el aviso de esa escalada al cliente |
| **`AUTOMATICO_EN_PAUSA`** | La misma `relevo_version`, con control humano | Cierre por confirmación del cliente durante la pausa de una escalada |
...
| **T2** | E1 → E2 | Tomar (operador) | `control_efectivo = 'humano'`, abierta, **sin asignación**; ya mía = no-op 200 (C16) | Asignación = yo, evento `tomada`. Asignada a otro: 409 `ya_asignada`. **No** resuelve `NO_DETERMINADO`. | Nada. |
| **T3** | E2 → E1 | Soltar (operador) | **Asignada a mí** (C16). Un `ADMIN` que necesita sacársela a alguien usa T4, no T3. | Asignación = ninguna, evento `soltada`; sigue humana. **No** resuelve `NO_DETERMINADO`. | Nada. |
| **T4** | E2(otro o libre) → E2(destino) | Reasignar | **Actor `ADMIN`** [AUDITORÍA]; `motivo` obligatorio; destino = persona activa de la organización; confirmación explícita; no sobre legado (C17). Habilitarlo entre pares por tenant queda para cuando una empresa lo pida. | Asignación = destino, evento `reasignada` {actor, anterior, nuevo, motivo, versión}. | Nada. |
...
| D4 | Dos operadores toman el mismo caso | `tomar_caso` sin condición | VERIFICADO | Resuelto en B3.4 (C16; T2, T4, §9.1): toma atómica sin pisar, soltar solo el dueño, reasignar ADMIN auditado |
...
| D25 | Una intervención concurrente no frenaba los **efectos** del turno de la IA: D24 descartaba la respuesta, pero una herramienta que escribe (o el agendamiento, el cierre, el ticket) podía empezar después de que el control ya era humano | `motor.responder` (despacho de herramientas), `atender_turno` | VERIFICADO (test con barrera, motor real) | Resuelto (C13) |
| D27 | `relevo_eventos.creado_en` usaba el default `now()`, la hora en que **empezó** la transacción: con dos transiciones concurrentes sobre la misma conversación, la que esperó el lock quedaba con una hora anterior habiendo escrito después (visto: `reasignada` v3 fechada antes que `tomada` v2) | `relevo_eventos` | VERIFICADO (sección 13) | Resuelto: el INSERT fija `creado_en = clock_timestamp()`, evaluado ya con la fila bloqueada. **El orden causal sigue siendo `datos.version`**; la hora mostrada ya no lo contradice. No se reescribe historia: lo anterior al arreglo conserva su `now()`. |
| D28 | La asignación del ticket en el CRM puede quedar desalineada de la asignación durable de Dexter: el proxy la sincroniza al tomar, y una reasignación concurrente deja el CRM con el dueño anterior. Hoy solo se sincroniza al **tomar** (nunca al soltar, y solo si la toma se aplicó); `reasignar` no toca el CRM | `api/conversaciones/[id]/atender/+server.js` | VERIFICADO (por lectura) | **Abierto, para B4/reconciliador.** Dexter es la fuente de verdad: la cola y el dueño de B3.5 usan la asignación local, nunca la del CRM |
| D24 | Un turno de la IA en vuelo sobrevivía a una intervención concurrente: la compuerta leía `control = ia`, el modelo tardaba, una persona intervenía (200) y la respuesta ya calculada se guardaba y se enviaba igual. No es D4 (operador contra operador): es IA contra cambio de control | `atender_turno`, `_procesar_mensaje_whatsapp` | VERIFICADO (test con barrera) | Resuelto (C10, C11) |
