...
| C13 | **D25. Ningún efecto que escribe, originado por un turno de la IA, empieza después de un cambio de control.** Mientras corre el turno, `atender_turno` instala un autorizador (`nucleo/relevo/autorizacion.py`); cada efecto le pregunta **justo antes de empezar** y el autorizador relee la base, fuera de transacción. Efecto **autónomo de la IA** (herramienta del modelo con `solo_lectura: false`, propuesta con aprobación humana, `proponer_herramienta`, visita agendada sola, cierre por confirmación): exige `control_efectivo = 'ia'` y la misma `relevo_version`. Efecto **de la escalada propia** (ticket operativo y caso del CRM de la escalada que ESTE turno comprometió; cierre confirmado durante la pausa): exige la versión que esa transición escribió, con control humano. Si no se cumple: no empieza, no se reintenta en el turno, la traza lo marca como bloqueo `CAMBIO_DE_CONTROL` (no fuerza escalada) y se cuenta `accion_ia_cancelada_por_cambio_de_control` sin contenido. Las lecturas no preguntan. Mismo punto de no retorno que C11. Fuera de un turno (acción aprobada por una persona, baterías) no hay autorizador. | T8, X3, C11 |
