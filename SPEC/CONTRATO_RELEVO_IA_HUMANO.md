...
| D27 | `relevo_eventos.creado_en` es `now()`, la hora en que **empezó** la transacción: con dos transiciones concurrentes sobre la misma conversación, la que esperó el lock puede quedar con una hora anterior y haber escrito después (visto: `reasignada` v3 antes que `tomada` v2). El orden de los eventos es `datos.version`; ningún lector ordena por `creado_en` (hoy no hay ninguno; la línea de tiempo de B3.5 debe respetarlo) | `relevo_eventos` | VERIFICADO (sección 13) | Regla fijada; lectores futuros |
