...
| **B4 · Q2** | [B4-Q2-WISPHUB.md](B4-Q2-WISPHUB.md) | **at most one POST** | `crear_ticket` no se reintenta — pero ya no por no poder buscar. Medido en vivo (Q2.1, 21/09): una referencia propia en `descripcion` sobrevive exacta, viene en el listado y se recupera por ventana acotada; WispHub NO deduplica. La política es asimétrica: **adoptar sí, recrear nunca**, y por eso no depende de inventar un timeout. |
...
| **B5** | [B5-ACCIONES.md](B5-ACCIONES.md) | **CERRADO EN CÓDIGO** | Aprobar es reservar, revalidar, ejecutar y resolver. La revalidación tiene tres desenlaces: «no se pudo comprobar» no ejecuta. Validador en modo advertencia hasta que cierre la medición (Q3). **21/09: `agregar_promesa_pago` dejaba que el modelo eligiera `accion`, y una opción reactivaba el servicio sin que el resumen lo nombrara — fija en 0 y fuera del esquema del modelo (`088b8d4`). La herramienta sigue SIN habilitar: §3.7 no es implementable, no hay API para «sin promesa vigente».** |
...
| **G7** | [G7-PREDEPLOY.md](G7-PREDEPLOY.md) | **listo para activación** | Las **cinco** capacidades declaradas: `asigna_caso` al endpoint aditivo y a ningún otro, `lee_caso` al GET del detalle. Ninguna herramienta usa PUT ni DELETE sobre un caso. Worker apagado por defecto, cadencia 300 s, y el dry-run medido en cero llamadas con control positivo. NO verde: falta aplicar config, desplegar y encender. |
...
| **B6** | [B6-CIERRE-DESENLACE.md](B6-CIERRE-DESENLACE.md) | **CERRADO EN CÓDIGO** | El cierre dice por qué: catálogo de doce códigos de plataforma, extensión por empresa con `categoria_base` obligatoria, una transición para los cinco caminos y otra para completar el desenlace después. El desenlace NO se infiere por ninguno. `cerrar_caso` va por B4 leyendo antes (un PATCH ciego reescribe `closed_on`, medido contra el CRM real); `cerrar_ticket` NO se automatiza: su única vía de cierre publica un comentario en cada intento. Desbloquea `cerrar_con_desenlace` en G8 y agrega la quinta capacidad de G7, `lee_caso`. |
