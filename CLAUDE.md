...
| D5 | **El segundo ISP no entra todavía, y la deuda crece.** El motor ya es multi-empresa (webhook por tenant, credenciales cifradas por empresa, aislamiento medido en 0 filas). El frontend no: `PRIVATE_ASISTENTE_TENANT` estaba en 23 lugares cuando se anotó y el **24/09/2026 son 70 archivos / 80 apariciones**, ninguna derivada de `locals.org` | Es una decisión de producto (¿una instalación por ISP, o una plataforma?) y conviene tomarla **antes** de escribir el código: cada pantalla nueva suma una aparición más |
